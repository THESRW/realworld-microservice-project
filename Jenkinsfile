...
pipeline {
    ...
    stages {
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker build -t srwansi257/frontendservice:latest ."
                    }
                }
            }
        }
        ...
        stage('Snyk SCA Test | Dependencies') {
            steps {
                sh "${SNYK_HOME}/snyk-linux test --docker srwansi257/frontendservice:latest || true" 
            }
        }
        ...
        stage('Push Microservice Docker Image') {
            steps {
                script {
                    withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'docker') {
                        sh "docker push srwansi257/frontendservice:latest "
                    }
                }
            }
        }
        ...
        //             sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.141.202.119 "docker run -t zaproxy/zap-weekly zap-baseline.py -t http://18.216.48.123:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#lesrw-77-multi-microservices-alerts', //update and provide your channel name
